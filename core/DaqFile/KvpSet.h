...
public: 
  ...
  std::string get(const std::string& key)                           { return mData[key]; }; 
  bool        has(const std::string& key)                           { return (mData.count(key) != 0); }
  int         getInt(const std::string& key, int default_val=0)       { return parseInt   (mData[key],default_val); };
  double      getDouble(const std::string& key, double default_val=0) { return parseDouble(mData[key],default_val); };
  std::string getString(const std::string& key, std::string default_val="") { return has(key)?get(key):default_val; };
